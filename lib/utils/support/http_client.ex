defmodule Phelper.HTTPClient do
  @moduledoc """
  HTTP client based on :httpc
  """

  @doc """
  iex> Phelper.HTTPClient.get "https://httpbin.org/get"
  """
  def get(url, opts \\ []) do
    request(:get, url, opts[:headers] || %{}, opts[:body] || "")
  end
  ...
  defp format_resp({:ok, {{http, status, _status_phrase}, headers, body}}) do
    json? =
      headers |> Enum.any?(fn {k, v} -> k == ~c"content-type" and v == ~c"application/json" end)

    body = if json?, do: JSON.decode!(body), else: body

    {:ok, %{http: http, status: status, headers: headers, body: body}}
  end
